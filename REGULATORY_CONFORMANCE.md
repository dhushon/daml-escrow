...
*   **Tripartite Authority:** Every escrow contract requires co-signatures from the **Depositor**, **Beneficiary**, and **Issuer (Bank)**. No single party can unilaterally move funds once locked.
...
*   **Cross-Node Reconciliation:** Regulators can query the separate nodes and reconcile the states to ensure no node has been tampered with. If the Bank node reports "SETTLED" but the Depositor node reports "ACTIVE," the system triggers a **Consensus Alert**.
...
*   **Atomic Transactions:** Commands in DAML are atomic. If a multi-party signature fails (e.g., the Beneficiary node is offline), the transaction is never committed.
*   **Timeout-Based Refund:** Every escrow includes a mandatory `ExpiryDate`. If a deadlock occurs or a node goes permanently offline, the Issuer node can authoritatively execute an "Expiry" choice to return funds to the Depositor after the deadline passes.
